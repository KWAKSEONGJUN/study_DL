...
class CNN(tf.keras.Model):
    def __init__(self):
        ...
        self.conv_layer1 = tf.keras.layers.Conv2D(filters=64,
                                                  ...
                                                  padding='same',
                                                  activation='relu')
        self.pool_layer1 = tf.keras.layers.MaxPool2D(pool_size=(3, 3),
                                                     strides=2)
        self.conv_layer2 = tf.keras.layers.Conv2D(filters=64,
                                                  ...
                                                  padding='same',
                                                  activation='relu')
        self.pool_layer2 = tf.keras.layers.MaxPool2D(pool_size=(3, 3),
                                                     strides=2)
        self.conv_layer3 = tf.keras.layers.Conv2D(filters=128,
                                                  ...
                                                  padding='same',
                                                  activation='relu')
        self.conv_layer4 = tf.keras.layers.Conv2D(filters=128,
                                                  kernel_size=3,
                                                  strides=1,
                                                  padding='same',
                                                  activation='relu')
        ...
        self.fc_layer = tf.keras.layers.Dense(384, activation='relu')
    ...
    def call(self, x, is_training):
        ...
        h_flat = self.flatten_layer(h_conv5)
        h_fc = self.fc_layer(h_flat)
        h_fc_drop = self.dropout(h_fc, training=is_training)
        logits = self.output_layer(h_fc_drop)
...
for i in range(10000):
    ...
    if i % 100 == 0:
        train_accuracy = compute_accuracy(CNN_model(batch_x, False), batch_y)
        train_loss = cross_entropy_loss(CNN_model(batch_x, False), batch_y)
        print('EPOCH : %d\t Accuracy : %f\t Loss :%f' % (i, train_accuracy, train_loss))

    train_step(CNN_model, batch_x, batch_y, True)

test_accuracy = 0.0
for i in range(10):
    batch_x, batch_y = next(test_data_iter)
    test_accuracy += compute_accuracy(CNN_model(batch_x, False), batch_y)
